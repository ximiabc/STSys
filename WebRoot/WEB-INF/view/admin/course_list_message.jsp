...
<html>
  ...
  <body>
   		<a href="adminaddcourse" class="btn btn-success">
		...
		<table class="table table-bordered table-header text-center">
			<thead><!-- 表头 -->
				<tr class="bolder">
					<td></td>
					<td class="w10">课程名</td>
					<td class="w15">排课班级</td>
					<td class="w15">试卷模板</td>
					<td class="w15">知识点</td>
					<td class="w15">操作</td>
				</tr>
			</thead>
			<tbody style="color: #000;"><!-- 表单体 -->
				<s:iterator value="pageBean.recordList" var="n" status="indexs">
					<tr>
						...
						<td>${name }</td>
						<td>查看：${classes.size() }</td>
						<td>查看：${papers.size() }</td>
						<td>查看：${knowTypes.size() }</td>
						<td>
							<a href="adminupdatecourse?id=${id }" class="btn btn-warning btn-sm">
								<i class="glyphicon glyphicon-wrench"></i>
							</a>
                          	<a href="admindelcourse?id=${id }" onclick="return confirm('确认删除此班级？');" class="btn btn-default btn-sm">
				...
				<tr>
					<td colspan="9">
						<div class="pull-right">
							<nav>
							<ul class="pagination">
								<li><a href="adminlistcourse?pageNum=0" aria-label="Previous">
								...
								<s:iterator begin="pageBean.beginPageIndex"
									...
									<li><c:if test="${pageBean.currentPage == pageNums }">
											...
											<a href="adminlistcourse?pageNum=${pageNums }">${pageNums }</a>
										</c:if></li>
								</s:iterator>
								<li><a href="adminlistcourse?pageNum=${pageBean.pageCount }"
