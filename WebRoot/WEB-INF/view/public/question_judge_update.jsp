  ...
  <body>
    ...
    <h3>添加单项选择题</h3>
  	<div class="text-center" style="padding-left: 50px;">
  		<form action="ajudgeupdates?id=${bean.id }" method="post">
  			<table>
  				<tbody>
  					<tr><td>（所属课程：
  					...
  					<tr><td colspan="2">答案：
  							<label>√<input type="radio" name="result" value="Y" <c:if test="${bean.result =='Y' }">checked</c:if> /></label>&nbsp;&nbsp;&nbsp;&nbsp;
		  					<label>×<input type="radio" name="result" value="N" <c:if test="${bean.result =='N' }">checked</c:if> /></label>
						</td> 
					</tr>
  				</tbody>
  				<tfoot>
