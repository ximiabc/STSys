...
<html>
  ...
  <body>
    	...
    	<span class="glyphicon glyphicon-menu-left"></span>
   		...
   		<a href="aselectadd" class="btn btn-success">
    	...
    	<div class="btn-group">
		  ...
		  <ul class="dropdown-menu">
		  	<s:iterator value="courseBean">
		  		<li><a href="aselectlistbyid?id=${id }">${name }</a></li>
		  	</s:iterator>
		  </ul>
		</div>
    	<div>
			...
			<tbody style="color: #000;"><!-- 表单体 -->
				<s:iterator value="pageBean.recordList" var="n" status="indexs">
					<tr>
						...
						<td><a href="aselectview?id=${id }">${title }</a></td>
						<td><a href="aknowlistById?id=${knowType.id }">${knowType.name }</a></td>
						<td>${course.name }</td>
						<td>${scoreMax }</td>
						<td>${stars }</td>
						<td>
							<a href="aselectupdate?id=${id }" class="btn btn-warning btn-sm">
								<i class="glyphicon glyphicon-wrench"></i>
							</a>
                          	<a href="aselectdel?id=${id }" onclick="return confirm('确认删除此知识类型？');" class="btn btn-default btn-sm">
				...
				<tr>
					<td colspan="9">
						<div class="pull-right">
							...
							<ul class="pagination">
								<c:if test="${listby == 0 }">
									<li><a href="aselectlist?pageNum=0" aria-label="Previous">
									...
									<s:iterator begin="pageBean.beginPageIndex"
										...
										<li><c:if test="${pageBean.currentPage == pageNums }">
												...
												<a href="aselectlist?pageNum=${pageNums }">${pageNums }</a>
											</c:if></li>
									</s:iterator>
									<li><a href="aselectlist?pageNum=${pageBean.pageCount }"
										aria-label="Next"> <span aria-hidden="true">&raquo;</span>
									</a></li>
								</c:if>
								<c:if test="${listby == 1 }">
									<li><a href="aselectlistbyid?id=${id }&pageNum=0" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span> </a></li>
									<s:iterator begin="pageBean.beginPageIndex"
										end="pageBean.endPageIndex" var="pageNums">
										<li><c:if test="${pageBean.currentPage == pageNums }">
												...
												<a href="aselectlistbyid?id=${id }&pageNum=${pageNums }">${pageNums }</a>
											</c:if></li>
									</s:iterator>
									<li><a href="aselectlistbyid?id=${id }&pageNum=${pageBean.pageCount }"
										aria-label="Next"> <span aria-hidden="true">&raquo;</span>
									</a></li>
								</c:if>
							</ul>
