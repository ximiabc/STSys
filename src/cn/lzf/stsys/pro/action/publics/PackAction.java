 ...
 *
 */
public class PackAction extends BaseAction<QuestionPack>{
	...
	//分页查询
	public String list() {
		PageBean pageBean=packsService.getPageBean(pageNum, 10, new HqlHelper(QuestionPack.class, "c")
	...
	//单选详情
	public String view() {
		QuestionPack single=packsService.getById(getModel().getId());
		ValueStack vs=ActionContext.getContext().getValueStack();
		vs.set("bean", single);
		return "view";
	}
	...
	public String add() {
		ValueStack vs=ActionContext.getContext().getValueStack();
		vs.set("bean", new QuestionPack());
		geCourse();
		return "add";
	}
	public String adds() {
		QuestionPack single=getModel();
	...
	public String update() {
		ValueStack vs=ActionContext.getContext().getValueStack();
		vs.set("bean", packsService.getById(getModel().getId()));
		geCourse();
		return "update";
	}
	...
	//删除
	public String del(){
		QuestionPack single=new QuestionPack();
		single.setId(getModel().getId());
		packsService.update(single);
		packsService.delete(getModel().getId());
		list();
		return "list";
	}
